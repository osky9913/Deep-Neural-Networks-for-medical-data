...
types_of_teeth = ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H']
selected_teeth = types_of_teeth[0]  # L
...
for x in range(angle, abs(angle), step):
    for y in range(angle, abs(angle), step):
        for z in range(angle, abs(angle), step):
            ...
            file = os.path.join(path, teeth_mask, targetName, selected_teeth, input,
            ...
            file = os.path.join(path, teeth_mask, targetName, selected_teeth, output_mask, targetName +
                                ('x' + str(x) + 'y' + str(y) + 'z' + str(z)))
            bpy.context.scene.render.filepath = file
            bpy.ops.render.opengl(write_still=True)
            bpy.ops.render.opengl(write_still=True)
